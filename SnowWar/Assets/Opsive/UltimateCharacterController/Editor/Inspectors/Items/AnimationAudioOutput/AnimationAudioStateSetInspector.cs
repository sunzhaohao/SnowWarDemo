                                                ...
                                                ReorderableList.RemoveCallbackDelegate stateRemoveCallback, string statePreferencesKey)
        {
            ...
            if (s_SelectorTypeNameCache != null) {
                ...
                if (newSelected != selected || forceUpdate) {
                    // Use the Random selector as the default (or recoil in the case of a melee weapon).
                    if (forceUpdate) {
                        for (int i = 0; i < s_SelectorTypeCache.Count; ++i) {
                            if ((randomDefaultSelector && s_SelectorTypeCache[i].FullName == "Opsive.UltimateCharacterController.Items.AnimatorAudioStates.Random") ||
